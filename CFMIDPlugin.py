...
class CFMIDPlugin:
 ...
 def output(self, outputfile):
  ...
  xls = pd.ExcelFile(PyPluMA.prefix()+"/"+self.parameters["excelfile"])
  ...
  for i in sheet_list:
    ...
    if counter == 1:
        ...
        df_temp = pd.read_excel(xls, i) ## Also tried sheet_name = 1
        df_temp['mixture'] = i
  ...
  ########################################################################
  df3 = pd.read_csv(PyPluMA.prefix()+"/"+self.parameters["compounds"])
  ...
  df_conv = pd.read_csv(PyPluMA.prefix()+"/"+self.parameters["cidfile"])
  ...
  ########################################################################
  os.chdir(PyPluMA.prefix()+"/"+self.parameters["searchfolder"]+"/")
  ...
  #os.chdir("C:/users/achao/OneDrive - Environmental Protection Agency (EPA)/profile/Desktop/pythontemp/CFMID code for manuscript submission/Results code")
  #os.chdir(PyPluMA.prefix())
  df1_cfmid_CE10.to_csv("../"+outputfile[outputfile.rfind('/')+1:], sep=',', encoding='utf-8', index=False)
